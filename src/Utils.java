...
public class Utils {
    ...
    public void moveEventStatus(String move){
        ...
        if(moveStatus==0){
            System.out.println("触摸状态为空置状态=="+moveStatus);
        }else if(moveStatus==1){
            System.out.println("触摸状态为触摸状态=="+moveStatus);
        }else if(moveStatus==2){
            System.out.println("触摸状态为移动状态=="+moveStatus);
        }else if(moveStatus==3){
            System.out.println("触摸状态为释放状态=="+moveStatus);
        }
    }
    /**
     *  解析触摸板x y坐标
     * @param move 16进制字符串
     */
    public void xOryMoveEvent(String move){
        moveResult=twoHexToInt(move);
        //截取计算x或y坐标值,前6位清零
        int xOryPoint=moveResult & 0x3ff;
        System.out.println("坐标的值为=="+xOryPoint);
    }
    /**
     * //Bit 7 6 5 4 3 2 1 0
    ...
    public  void keyDownEvent(String keyDown){
        ...
        for (int i = 0; i <keyDownToBin.length(); i++) {
            ...
            if(key==1){//
                switch(i){
                    case 4:
                        System.out.println("App 按键按下....");
                        break;
                    case 3:
                        System.out.println("Power/Home 按键按下....");
                        break;
                    case 2:
                        System.out.println("Touchpad 按键按下....");
                        break;
                    case 1:
                        System.out.println("Hand (left) 按键按下....");
                        break;
                    case 0:
                        System.out.println("Hand (right 按键按下....");
                        break;
                }
     ...
     * @param dataType
     */
    public  void dataTypeEvent(String dataType){
        dataTypeResult=oneHexToInt(dataType);
        System.out.println("dataTypeResult=="+dataTypeResult);
        if(dataTypeResult==1){
            System.out.println("数据类型为欧拉角类型");
        }else if(dataTypeResult==2){
            System.out.println("数据类型为四元数类型");
        }else if(dataTypeResult==32){
            System.out.println("数据类型为欧拉角与加速度与陀螺仪类型");
        }
    }
    ...
    public void triggerEvent(String trigger){
        ...
        if(triggerResult==0){
            System.out.println("扳机松开...");
        }else if(triggerResult==255){
            System.out.println("扳机按下...");
        }
    }
    ...
    public  String getCoordinatesResult(String coordinaStr){
        ...
        binStr = Integer.toBinaryString( strToChar[0] );
        if(fuhaowei==0){
            fuhaoweiIsZhen=true;
        }else if(fuhaowei==1){
